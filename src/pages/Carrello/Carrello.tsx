...
import { aggiungiAllOrdine, rimuoviDalCarrello, svuotaCarrello } from '../../store/actionCreators';
...
function Carrello(){
    ...
    return(
        <>
            ...
            <div id="link-ordine">
                <NavLink to={'/riepilogo-ordine'}
                    style={({ isActive }) => ({
                        textDecoration: "none",
                        fontSize: "large",
                        color: "white"
                    })}>
                    Riepilogo Ordine</NavLink>
            </div>
            <div>
                <h2>Carrello</h2>
                {
            ...
            <div>
                <h4>Totale: € {carrello.reduce((acc, el) => acc + el.price * el.quantita, 0).toFixed(2)}</h4>
                <button id="btn-acquista" onClick={() => addToOrderlist(carrello)}>Acquista</button>
            </div>
        </>
    )
}
