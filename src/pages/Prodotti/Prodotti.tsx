...
function Prodotti() {
    ...
    const products : Product[] = useSelector((state: RootState) => state.general.prodotti);
    const user: Credentials | null = useSelector((state:RootState) => state.general.user);

    useEffect(()=>{
        if(products.length === 0)
        dispatch(chiamataProdotti())
    },[])
    ...
    return (
        <>
        {
            ...
            <>
                ...
                <div id='link-new'>
                <NavLink to={'/nuovo-prodotto'}
                            style={({ isActive }) => ({
                                textDecoration: "none",
                                color: isActive ? "purple" : "grey",
                                fontSize: "large"
                            })}>
                            Aggiungi nuovo prodotto</NavLink>
                </div>
            </>
            :
            <>
